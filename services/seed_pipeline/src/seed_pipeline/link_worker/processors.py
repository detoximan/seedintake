...
class VideoFrameOCRExtractor:
    """Извлекает кадры из видео и прогоняет через OCR для распознавания наложенного текста."""
    def __init__(self, ocr: ImageOcrExtractor, frame_interval: float = 2.0):
        self.ocr = ocr
        self.frame_interval = frame_interval

    def extract_text(self, video_path: Path, workdir: Path) -> str:
        import re
        frames_dir = workdir / "frames_ocr"
        ...
        try:
            ...
            logger.warning("Failed to extract frames from %s: %s", video_path.name, e)
            return ""

        texts = []
        for frame_path in sorted(frames_dir.glob("frame_*.jpg")):
            text = self.ocr.extract(frame_path)
            if text and not text.startswith("(пусто)") and not text.startswith("(ошибка"):
                texts.append(text)
        # Дедупликация
        seen = set()
        unique_texts = []
...
class YtDlpAudioDownloader:
    ...
    def download_video(self, url: str, target_dir: Path) -> Path | None:
        """Скачивает видео для последующего OCR кадров."""
        if shutil.which(self.executable) is None:
            logger.warning("yt-dlp not found, skipping video download for OCR")
            return None
        target_dir.mkdir(parents=True, exist_ok=True)
        command = [
        ...
        command.append(url)
        try:
            completed = subprocess.run(command, capture_output=True, check=False, text=True, timeout=120)
        except subprocess.TimeoutExpired:
            logger.warning("yt-dlp video download timed out")
            return None
        if completed.returncode != 0:
            logger.warning("yt-dlp video download failed")
            return None
        video_files = [p for p in target_dir.iterdir() if p.is_file() and p.suffix in {'.mp4', '.mkv', '.webm'}]
