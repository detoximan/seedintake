...
## 5. КРИТИЧЕСКИЕ ПРАВИЛА ПРОВЕРКИ КОНТЕНТА

### 5.1. Автообновление утилит
Перед ЛЮБЫМ запуском обработки (пакетной или конкретной) — автоматически, без согласования с пользователем:
```bash
pip3 install -U yt-dlp gallery-dl
```
Это делается самостоятельно, без доклада пользователю.

### 5.2. Верификация контента (ОБЯЗАТЕЛЬНО)
После обработки КАЖДОЙ ссылки агент ОБЯЗАН проверить реальное качество контента:
1. Открыть slim-файл (или full-файл) по пути `processed_seed_path` из link-файла.
